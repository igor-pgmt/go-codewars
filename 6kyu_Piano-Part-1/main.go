...
func main() {
	...
	println(BlackOrWhiteKey1(2017))
	println(BlackOrWhiteKey2(2017))

}

func BlackOrWhiteKey1(keyPressCount int) string {

	b := "black"
	w := "white"

	octave := [12]string{w, b, w, w, b, w, b, w, w, b, w, b}
	return octave[((keyPressCount-1)%88)%12]

}

// "Индусский код"
func BlackOrWhiteKey2(keyPressCount int) string {
	// Indexes of black keys in single octave
	blacks := [5]int{2, 5, 7, 10, 12}

	// Amount of keys in single keyboard
	keyboardKeys := 88
	// Amount of keys in single octave
	octaveKeys := 12

	// check for white key
	if (keyPressCount % keyboardKeys) == 0 {
		return "white"
	}

	// Reduction to single keyboard
	singleKeyboyard := int(keyPressCount / keyboardKeys)
	newKey := keyPressCount - singleKeyboyard*keyboardKeys

	// Check for black key
	if (newKey % octaveKeys) == 0 {
		return "black"
	}

	// Reduction to single octave
	octaves := int(newKey / octaveKeys)
	key := newKey - octaves*octaveKeys
